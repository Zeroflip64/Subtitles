...
def subtitles_to_vector(subtitles, doc2vec_model):
    return doc2vec_model.infer_vector(subtitles)
...
def preparing(data, preprocesing):
    ...
    data['Vec'] = data['subs'].apply(lambda x: subtitles_to_vector(x, doc2vec_model))
...
if 'RUS' in selected_len:
  ...
  if uploaded_file is not None:
    ...
    predict=label.inverse_transform(model.predict(predict))
